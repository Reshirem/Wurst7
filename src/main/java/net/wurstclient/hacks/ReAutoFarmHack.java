...
@SearchTags({"auto farm", "re"})
public final class ReAutoFarmHack extends Hack implements UpdateListener, RenderListener {
    private final SliderSetting range = new SliderSetting("Range", 5, 1, 6, 0.05, SliderSetting.ValueDisplay.DECIMAL);
    private final SliderSetting rangez = new SliderSetting("RangeZ", 3.9, 1, 6, 0.05, SliderSetting.ValueDisplay.DECIMAL);
    ...
    public ReAutoFarmHack() {
        super("ReAutoFarm", "狗re自用的地狱疣farm功能");
    ...
    private Stream<BlockPos> getBlockStream(BlockPos center, int range, int rangez) {
        BlockPos min = center.add(-range, -1, -rangez);
        BlockPos max = center.add(range, 1, rangez);
    ...
    private static boolean breakWart(BlockPos pos) {
        // set slot
        MC.player.inventory.selectedSlot = 0;
        boolean a = BlockBreaker.breakOneBlock(pos);
        return a;
    }
}
